...
class FcaBottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        # c2wh 是一个字典，其中包含了与不同通道数（channel number）对应的特定宽度（width）和高度（height）的映射通道数 64 对应 宽度和高度 56。
        # 通道数 128 对应 宽度和高度 28
...
def fcanet34(num_classes=17, pretrained=False):
...
def fcanet50(num_classes=17, pretrained=False):
...
def fcanet101(num_classes=17, pretrained=False):
...
def fcanet152(num_classes=17, pretrained=False):
    """Constructs a FcaNet-101 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
